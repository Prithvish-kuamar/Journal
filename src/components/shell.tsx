...
const navGroups: NavGroup[] = [
  ...
  { label: "JOURNALING", items: [{ label: "Dashboard", href: "/", icon: "◈" }, { label: "Comparison", href: "/?tab=comparison", icon: "⊞" }, { label: "Analysis", href: "/?tab=analysis", icon: "◉" }, { label: "Trades", href: "/journal", icon: "↗" }, { label: "Reviews", href: "/review", icon: "✓" }, { label: "Periodic Logs", href: "/analytics", icon: "≡" }] },
  { label: "STRATEGY", items: [{ label: "Strategy Library", href: "/strategy", icon: "▤" }, { label: "Entry Models", href: "/strategy", icon: "⊹" }, { label: "Rulebook", href: "/strategy", icon: "☷" }] },
  { label: "SYSTEM", items: [{ label: "Settings", href: "/settings", icon: "⚙" }, { label: "Help", href: "/help", icon: "?" }] }
];

const titles: Record<string, string> = { "/": "Journaling Dashboard", "/plan": "Daily Plan", "/journal": "Setup Journal", "/review": "Post-trade Review", "/strategy": "Strategy Library", "/analytics": "Periodic Logs", "/settings": "Settings", "/help": "Help" };
...
export function Shell({ children, hideNewSetup = false }: { children: React.ReactNode; hideNewSetup?: boolean }) {
  ...
  return <div className="shell">
    ...
    <section className="content"><header className="topbar"><div className="topbar-title">{title}</div><div className="topbar-actions"><span className="topbar-status" title="Local journal is available. Broker synchronization is not implemented.">Local journal</span><Link className="topbar-action" href="/settings" aria-label="Open settings">⚙</Link><Link className="topbar-action" href="/review" aria-label="Open review queue">◉</Link>{!hideNewSetup && <Link className="new" href="/journal/new">+ New Setup</Link>}<form action={logout} className="topbar-logout"><button className="topbar-action" aria-label="Log out" title="Log out">↪</button></form></div></header><main className="page">{children}</main></section>
  </div>;
}
