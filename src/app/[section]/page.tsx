...
export default async function LaterPhase({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "settings") {
    ...
    const current = draftConfig.customTargetPrice != null ? String(draftConfig.customTargetPrice) : "";
    return <Shell>
      ...
      <section className="card">
        ...
        <p className="muted">Current published value: {currentPublished || "Not set"}</p>
        {draft ? <form action={saveCustomTargetPrice}>
          ...
          <p className="muted" style={{ fontSize: "0.8125rem" }}>Draft v{draft.versionNumber}. Save then <Link href="/strategy">publish the draft</Link> to promote this setting.</p>
          <button>Save</button>
        </form> : <p className="muted">No draft strategy version exists. <Link href="/strategy">Open Strategy Library</Link> to create one.</p>}
      </section>
    </Shell>;
  }
