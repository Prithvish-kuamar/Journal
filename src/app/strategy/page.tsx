...
import { DeleteVersionButton } from "@/app/strategy/delete-version-button";
...
export default async function StrategyPage() {
  ...
  return <Shell>
    <div className={styles.strategy}>
      ...
      <div className={styles.modelGrid}>{modelCards.map(({ model, trades, modelR, modelAccuracy }) => <article className={styles.modelCard} key={model.id}><header><StatusBadge tone={model.active ? "success" : "neutral"}>{model.active ? "Active" : "Inactive"}</StatusBadge><span>{model.code}</span></header><h3>{model.name}</h3><p>{model.shortDescription}</p><div className={styles.modelStats}><span><small>Executed R</small><b className={modelR >= 0 ? styles.positive : styles.negative}>{r(modelR)}</b></span><span><small>Accuracy</small><b>{modelAccuracy == null ? "—" : `${modelAccuracy}%`}</b></span><span><small>Trades</small><b>{trades.length}</b></span></div><Link href="/strategy/edit">Open model configuration</Link></article>)}</div>
