...
export default async function JournalPage({ searchParams }: { searchParams: Promise<{ date?: string; month?: string }> }) {
  ...
  const candidates = await prisma.setupCandidate.findMany({ where: { ownerId }, include: { gateAssessment: true, grade: true, trade: true, strategyVersion: true }, orderBy: { updatedAt: "desc" } });
