...
/**
 * Returns the signed-in user's id. Every lookup below takes a record id
 * straight from the submitted form, so each one must be constrained by this
 * value as well — otherwise any account could act on another account's rows
 * just by posting their id.
 */
...
export async function createStrategy(formData: FormData) {
  ...
  const strategy = await prisma.strategy.create({ data: { ownerId, name: input.name, description: input.description || null, versions: { create: { ownerId, versionNumber: 1, status: StrategyStatus.DRAFT, configuration: JSON.stringify(defaultStrategyConfiguration), changeSummary: "Initial owner configuration" } } }, include: { versions: true } });
  const version = strategy.versions[0];
  await audit(ownerId, "Strategy", strategy.id, "STRATEGY_CREATED", undefined, undefined, { versionId: version.id });
...
export async function addGateDefinition(formData: FormData) {
  ...
  const version = await prisma.strategyVersion.findFirstOrThrow({ where: { id: input.strategyVersionId, ownerId } });
  ...
  await audit(ownerId, "GateDefinition", input.gateKey, "GATE_DEFINITION_ADDED");
...
export async function addEntryModel(formData: FormData) {
  ...
  await audit(ownerId, "EntryModel", input.code, "ENTRY_MODEL_ADDED");
  revalidatePath("/strategy/edit");
}
...
export async function addGradeCategory(formData: FormData) {
  ...
  await audit(ownerId, "GradeCategory", input.categoryKey, "GRADE_CATEGORY_ADDED");
  revalidatePath("/strategy/edit");
}

async function audit(ownerId: string, entityType: string, entityId: string, action: string, reason?: string, previous?: object, next?: object) {
  await prisma.auditEvent.create({ data: { ownerId, entityType, entityId, action, reason, previous: previous ? JSON.stringify(previous) : null, next: next ? JSON.stringify(next) : null } });
}
...
export async function createDraftVersion(formData: FormData) {
  ...
  const current = await prisma.strategyVersion.findFirstOrThrow({ where: { id: versionId, ownerId }, include: { rules: true, gates: true, gradeCategories: true, entryModels: true, emotionalQuestions: true, journalOptions: true, instrumentMetadata: true } });
  const draft = await prisma.strategyVersion.create({ data: { ownerId, strategyId: current.strategyId, versionNumber: current.versionNumber + 1, parentVersionId: current.id, status: StrategyStatus.DRAFT, configuration: current.configuration, changeSummary: "Draft copied from published version", rules: { create: current.rules.map(withoutVersionFields) }, gates: { create: current.gates.map(withoutVersionFields) }, gradeCategories: { create: current.gradeCategories.map(withoutVersionFields) }, entryModels: { create: current.entryModels.map(withoutVersionFields) }, emotionalQuestions: { create: current.emotionalQuestions.map(withoutVersionFields) }, journalOptions: { create: current.journalOptions.map(withoutVersionFields) }, instrumentMetadata: { create: current.instrumentMetadata.map(withoutVersionFields) } } });
  await audit(ownerId, "StrategyVersion", draft.id, "STRATEGY_VERSION_DRAFT_CREATED", undefined, { source: current.id }, { version: draft.versionNumber });
  revalidatePath("/strategy");
}
...
export async function publishStrategyVersion(formData: FormData) {
  const ownerId = await requireOwnerId();
  const versionId = z.string().parse(formData.get("versionId"));
  await prisma.strategyVersion.findFirstOrThrow({ where: { id: versionId, ownerId }, select: { id: true } });
  ...
  await audit(ownerId, "StrategyVersion", version.id, "STRATEGY_VERSION_PUBLISHED", version.changeSummary ?? undefined);
  revalidatePath("/strategy");
}
...
export async function updateStrategyConfiguration(formData: FormData) {
  ...
  const version = await prisma.strategyVersion.findFirstOrThrow({ where: { id: versionId, ownerId } });
  ...
  await audit(ownerId, "StrategyVersion", versionId, "STRATEGY_CONFIGURATION_CHANGED");
  revalidatePath("/strategy");
}
...
export async function updateGateDefinition(formData: FormData) {
  ...
  const gate = await prisma.gateDefinition.findFirstOrThrow({ where: { id: gateId, strategyVersion: { ownerId } }, include: { strategyVersion: true } });
  ...
  await audit(ownerId, "GateDefinition", gateId, "GATE_DEFINITION_CHANGED");
  revalidatePath("/strategy");
}
...
export async function updateEmotionalQuestion(formData: FormData) {
  ...
  const question = await prisma.emotionalQuestion.findFirstOrThrow({ where: { id: questionId, strategyVersion: { ownerId } }, include: { strategyVersion: { include: { emotionalQuestions: true } } } });
  ...
  await audit(ownerId, "EmotionalQuestion", questionId, "EMOTIONAL_QUESTION_CHANGED");
  revalidatePath("/strategy/edit");
}
...
export async function addEmotionalQuestion(formData: FormData) {
  ...
  await audit(ownerId, "EmotionalQuestion", question.id, "EMOTIONAL_QUESTION_ADDED");
  revalidatePath("/strategy/edit");
}
...
export async function updateGradeCategory(formData: FormData) {
  ...
  const category = await prisma.gradeCategory.findFirstOrThrow({ where: { id: categoryId, strategyVersion: { ownerId } }, include: { strategyVersion: true } });
  ...
  await audit(ownerId, "GradeCategory", categoryId, "GRADE_CATEGORY_CHANGED");
  revalidatePath("/strategy");
}
...
export async function updateEntryModel(formData: FormData) {
  ...
  const model = await prisma.entryModel.findFirstOrThrow({ where: { id: modelId, strategyVersion: { ownerId } }, include: { strategyVersion: true } });
  ...
  await audit(ownerId, "EntryModel", modelId, "ENTRY_MODEL_CHANGED");
  revalidatePath("/strategy");
}
...
export async function saveDailyPlan(formData: FormData) {
  ...
  await prisma.strategyVersion.findFirstOrThrow({ where: { id: input.strategyVersionId, ownerId }, select: { id: true } });
  const plan = await prisma.dailyPlan.create({ data: { ownerId, strategyVersionId: input.strategyVersionId, account: input.account, planDate: new Date(input.planDate), sessionLabels: JSON.stringify([input.sessionLabel]), riskMode: input.riskMode, reducedRiskReason: input.riskMode === "REDUCED" ? String(formData.get("reducedRiskReason") || "") : null, objective: input.objective || null, readinessNotes: input.readinessNotes || null, myScenario: input.myScenario || null, noTradeConditions: String(formData.get("noTradeConditions") || "") || null, maxTradeTheses: 2, status: "ACTIVE" } });
  await audit(ownerId, "DailyPlan", plan.id, "DAILY_PLAN_CREATED");
  revalidatePath("/plan");
}
...
export async function upsertInstrumentPlan(formData: FormData) {
  ...
  await prisma.dailyPlan.findFirstOrThrow({ where: { id: dailyPlanId, ownerId }, select: { id: true } });
  ...
  if (id) {
    await prisma.instrumentPlan.findFirstOrThrow({ where: { id, dailyPlan: { ownerId } }, select: { id: true } });
...
export async function saveEndOfDay(formData: FormData) {
  ...
  await prisma.dailyPlan.findFirstOrThrow({ where: { id: planId, ownerId }, select: { id: true } });
  ...
  await audit(ownerId, "DailyPlan", planId, "DAILY_PLAN_COMPLETED");
  revalidatePath("/plan");
}
...
export async function createCandidate(formData: FormData) {
  ...
  const selectedVersion = await prisma.strategyVersion.findFirst({ where: { id: input.strategyVersionId, ownerId }, select: { id: true, status: true } });
  ...
  const requestedPlanId = String(formData.get("dailyPlanId") || "") || null;
  const linkedPlanId = requestedPlanId && await prisma.dailyPlan.findFirst({ where: { id: requestedPlanId, ownerId }, select: { id: true } }) ? requestedPlanId : null;
...
export async function saveTargetDecision(_: TargetDecisionResult, formData: FormData): Promise<TargetDecisionResult> {
  ...
  await prisma.setupCandidate.findFirstOrThrow({ where: { id: candidateId, ownerId }, select: { id: true } });
  ...
  try {
    ...
    if (!existing.some((target) => target.label === targetLabel)) {
      ...
      await audit(ownerId, "SetupCandidate", candidateId, "TARGET_DECISION_SAVED", undefined, undefined, { decision, label: targetLabel });
    }
  } catch (error) {
...
export async function answerGate(formData: FormData) {
  ...
  const candidate = await prisma.setupCandidate.findFirstOrThrow({ where: { id: candidateId, ownerId }, include: { gateAssessment: { include: { responses: true } }, strategyVersion: { include: { gates: { where: { active: true }, orderBy: { displayOrder: "asc" } } } } } });
  ...
  await audit(ownerId, "GateAssessment", assessment.id, !answer ? "GATE_REJECTED" : "GATE_RESPONSE_CHANGED", undefined, undefined, { gateKey, answer, outcome });
  revalidatePath(`/journal/${candidateId}`);
  redirect(`/journal/${candidateId}`);
}

async function todayLimitStatus(ownerId: string, account: string) {
  const startOfDay = new Date();
  startOfDay.setHours(0, 0, 0, 0);
  // Scoped by ownerId: account labels are free text and collide across accounts.
  const trades = await prisma.trade.findMany({ where: { ownerId, account, entryTimestamp: { gte: startOfDay } }, orderBy: { entryTimestamp: "desc" }, select: { status: true, netResult: true } });
...
export async function answerEmotionalQuestion(formData: FormData) {
  ...
  const candidate = await prisma.setupCandidate.findFirstOrThrow({
  ...
  const limits = await todayLimitStatus(ownerId, candidate.account);
  ...
  if (outcome.result === "REJECTED") await prisma.setupCandidate.update({ where: { id: candidateId }, data: { lifecycle: "REJECTED" } });
  if (outcome.result === "PASSED") await prisma.setupCandidate.update({ where: { id: candidateId }, data: { lifecycle: "QUALIFIED" } });
  await audit(ownerId, "EmotionalAssessment", emotionalAssessment.id, hardFailure ? "EMOTIONAL_HARD_LIMIT_FAILED" : !answer ? "EMOTIONAL_READINESS_FAILED" : "EMOTIONAL_READINESS_RESPONSE", hardFailure ? "Daily two-trade or two-loss limit reached." : undefined, undefined, { questionId, answer, summary, limits });
  if (summary.result === "REJECTED") await audit(ownerId, "GateAssessment", assessment.id, "GATE_REJECTED", "Gate 15 emotional-readiness failure", undefined, { firstFailedQuestionId: summary.firstFailedQuestionId });
  revalidatePath(`/journal/${candidateId}`);
  redirect(`/journal/${candidateId}`);
}
...
export async function completeDiagnostics(formData: FormData) {
  const ownerId = await requireOwnerId();
  const candidateId = z.string().parse(formData.get("candidateId"));
  const assessment = await prisma.gateAssessment.findFirstOrThrow({ where: { candidateId, candidate: { ownerId } } });
  await prisma.gateAssessment.update({ where: { id: assessment.id }, data: { diagnosticCompletion: "COMPLETE" } });
  await audit(ownerId, "GateAssessment", assessment.id, "DIAGNOSTIC_REVIEW_COMPLETED");
  revalidatePath(`/journal/${candidateId}`);
  redirect(`/journal/${candidateId}`);
}
...
export async function saveGrade(formData: FormData) {
  const ownerId = await requireOwnerId();
  const candidateId = z.string().parse(formData.get("candidateId"));
  const candidate = await prisma.setupCandidate.findFirstOrThrow({ where: { id: candidateId, ownerId }, include: { gateAssessment: true, grade: true } });
  ...
  await audit(ownerId, "SetupGrade", candidateId, "SETUP_GRADED", undefined, undefined, grade);
  revalidatePath(`/journal/${candidateId}`);
  redirect(`/journal/${candidateId}`);
}
...
export async function createTrade(formData: FormData) {
  const ownerId = await requireOwnerId();
  const candidateId = z.string().parse(formData.get("candidateId"));
  const candidate = await prisma.setupCandidate.findFirstOrThrow({ where: { id: candidateId, ownerId }, include: { grade: true, targets: true, gateAssessment: { include: { emotionalAssessment: true } } } });
  ...
  await audit(ownerId, "Trade", trade.id, restricted ? "RESTRICTED_TRADE_RECORDED" : "TRADE_CREATED", overrideReason || undefined);
  await audit(ownerId, "SetupGrade", candidateId, "SETUP_GRADE_LOCKED");
  revalidatePath(`/journal/${candidateId}`);
  revalidatePath("/journal");
}
...
export async function addTradeLeg(formData: FormData) {
  ...
  await prisma.trade.findFirstOrThrow({ where: { id: tradeId, ownerId }, select: { id: true } });
  ...
  await audit(ownerId, "TradeLeg", leg.id, input.planned ? "ADD_ON_CREATED" : "ADD_ON_MARKED_UNPLANNED");
...
export async function closeTrade(formData: FormData) {
  const ownerId = await requireOwnerId();
  const tradeId = z.string().parse(formData.get("tradeId"));
  const trade = await prisma.trade.findFirstOrThrow({ where: { id: tradeId, ownerId }, select: { id: true, candidateId: true, status: true, executedRisk: true, entryTimestamp: true, instrument: true, direction: true, entryPrice: true, positionSize: true, strategyVersionId: true } });
...
export async function saveCustomTargetPrice(formData: FormData) {
  ...
  await audit(ownerId, "StrategyVersion", versionId, "CUSTOM_TARGET_PRICE_CHANGED", undefined, { customTargetPrice: previousPrice }, { customTargetPrice: config.customTargetPrice });
...
export async function deleteStrategyVersion(formData: FormData) {
  ...
  await audit(ownerId, "StrategyVersion", versionId, "STRATEGY_VERSION_DELETED", undefined, { previousCounts });
